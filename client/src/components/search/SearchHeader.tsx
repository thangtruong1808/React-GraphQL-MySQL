        ...
        {totalResults > 0 ? (
          <div className="space-y-4">
            ...
            {((searchType.isUserSearch ? 1 : 0) +
              ...
              (searchType.isTaskStatusSearch ? 1 : 0) +
              (searchType.isRoleSearch ? 1 : 0)) > 1 && (
            ...
            <div className={`grid gap-4 ${
              ...
              (searchType.isUserSearch ? 1 : 0) +
                ...
                : (searchType.isUserSearch ? 1 : 0) +
                  (searchType.isProjectStatusSearch ? 1 : 0) +
                  (searchType.isTaskStatusSearch ? 1 : 0) +
                  (searchType.isRoleSearch ? 1 : 0) === 2
                  ? 'grid-cols-1 md:grid-cols-2'
                  : (searchType.isUserSearch ? 1 : 0) +
                    (searchType.isProjectStatusSearch ? 1 : 0) +
                    (searchType.isTaskStatusSearch ? 1 : 0) +
                    (searchType.isRoleSearch ? 1 : 0) === 3
              ...
              {/* Members count - Show only when searching by role */}
              {searchType.isRoleSearch && memberCount > 0 && (
                <div className="flex items-center justify-center p-4 bg-gradient-to-br from-purple-50 to-purple-100 rounded-2xl border border-purple-200">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="text-2xl font-bold text-purple-700">{memberCount}</p>
                      <p className="text-sm text-purple-600 font-medium">{memberCount === 1 ? 'Member' : 'Members'}</p>
                    </div>
                  </div>
                </div>
              )}
