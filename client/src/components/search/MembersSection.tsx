...
const MembersSection: React.FC<MembersSectionProps> = ({
  ...
  const renderMember = (member: Member) => (
    <div
      key={member.id}
      className="group bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-lg hover:border-blue-300 transition-all duration-300 transform hover:-translate-y-1 cursor-pointer"
      onClick={() => handleMemberClick(member)}
    >
      <div className="flex items-start space-x-5">
        {/* Enhanced Member avatar with status indicator */}
        <div className="flex-shrink-0 relative">
          <div className="h-16 w-16 bg-gradient-to-br from-blue-500 to-indigo-500 rounded-full flex items-center justify-center shadow-lg group-hover:shadow-xl transition-shadow duration-300">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mb-4">
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 flex items-center">
              <svg className="h-3 w-3 mr-1 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="mb-4">
            ...
            <div className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg group-hover:bg-blue-50 transition-colors duration-200">
              ...
              <p className="text-sm text-gray-700 font-medium group-hover:text-blue-700 transition-colors duration-200">{member.email || 'No email provided'}</p>
            </div>
          </div>
