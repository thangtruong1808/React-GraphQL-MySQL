...
export const useActivityTracker = () => {
  ...
  const handleUserActivity = useCallback(async () => {
    try {
      ...
      if (isModalShowing) {
        // Activity tracker: Session expiry modal showing - skipping activity update
        return;
      }
      ...
      if (tokens.accessToken) {
        ...
        if (isAccessTokenExpired) {
          // Activity tracker: Access token expired - skipping activity update to prevent timer reset
          return;
        }
      }
      
      // Step 3: Check if app is focused
      if (!isAppFocusedRef.current) {
        // Activity tracker: App not focused, skipping
        return;
      }

      // Step 4: Get refresh token status asynchronously to avoid race conditions
      const refreshTokenStatus = await TokenManager.getRefreshTokenStatus();
      
      // Step 5: Check if refresh token timer is actively counting down
      ...
      // Step 6: Skip activity updates only during refresh token countdown (not during transitions)
